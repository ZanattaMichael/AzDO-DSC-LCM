...
ForEach ($task in $PipelineResources)
{
    ...
    if ($null -eq $task.properties) {
        Write-Host "[Invoke-DscConfiguration] 'Properties' key does not exist for resource: [$($task.type)/$($task.name)]" -ForegroundColor Red
        $isFail = $true
    }

    # Throw an error is the name key does not exist
    if ($null -eq $task.name) {
        Write-Host "[Invoke-DscConfiguration] 'Name' key does not exist for resource: [$($task.type)/$($task.name)]" -ForegroundColor Red
        $isFail = $true
    }
    ...
    if ($null -eq $task.type) {
        Write-Host "[Invoke-DscConfiguration] 'Type' key does not exist for resource: [$($task.type)/$($task.name)]" -ForegroundColor Red
        $isFail = $true
    }
    #>
    ...
    if ($isFail) {
        Write-Host "[Invoke-DscConfiguration] Skipping [$($task.type)/$($task.name)]" -ForegroundColor Yellow
        break
    }
    ...
    if ($null -eq $resource) {
        Write-Host "[Invoke-DscConfiguration] Resource [$resourceType] was not found in module [$module]" -ForegroundColor Red
        $isFail = $true
    }
    ...
    ForEach ($property in $properties.keys)
    {
        ...
        if ($Property -notin $resource.Properties.Name) {
            Write-Host "[Invoke-DscConfiguration] Property [$($property)] does not exist in resource [$resourceType] in module [$module]" -ForegroundColor Red
            $isFail = $true
        }
        ...
        if ($configurationPropertyValue.GetType().Name -ne $resourcePropertyType) {
            Write-Host "[Invoke-DscConfiguration] Property [$($property)] is not the correct type in resource [$resourceType] in module [$module]" -ForegroundColor Red
            $isFail = $true
        }
        #>
        ...
        if ($resourceProperty.IsMandatory -and $null -eq $configurationPropertyValue) {
            Write-Host "[Invoke-DscConfiguration] Property [$($property)] is mandatory in resource [$resourceType] in module [$module]" -ForegroundColor Red
            $isFail = $true
        }

        # If the Property is a caculated variable, ignore the property
        if ($configurationPropertyValue -match "\$") {
            Write-Host "[Invoke-DscConfiguration] Property [$($property)] is a caculated variable in resource [$resourceType] in module [$module]" -ForegroundColor Yellow
            continue
        }
        ...
        if ($resourceProperty.Values -and $configurationPropertyValue -notin $resourceProperty.Values) {
            Write-Host "[Invoke-DscConfiguration] Property [$($property)] with the value [$($configurationPropertyValue)] does not match the selected values in resource [$resourceType][$property]. Values can be: $($resourceProperty.Values -join ',')" -ForegroundColor Red
            $isFail = $true
        }
